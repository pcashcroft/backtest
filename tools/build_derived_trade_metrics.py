...
How to run:
  ...
  C:\\Users\\pcash\\anaconda3\\envs\\backtest\\python.exe tools\\build_derived_trade_metrics.py

  # Force-rebuild all dates:
  C:\\Users\\pcash\\anaconda3\\envs\\backtest\\python.exe tools\\build_derived_trade_metrics.py --force-rebuild

  # Only process one specific dataset (e.g. just footprint):
  C:\\Users\\pcash\\anaconda3\\envs\\backtest\\python.exe tools\\build_derived_trade_metrics.py --dataset-id ES_FOOTPRINT_1M
...
- "No derived_trade_metrics rows found": run tools/add_trade_metrics_config.py first.
...
def main() -> int:
    ...
    if not derived_rows:
        print(
            ...
            "Run tools/add_trade_metrics_config.py first.",
            file=sys.stderr,
        )
        return 1
