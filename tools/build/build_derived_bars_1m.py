...
How to run:
  ...
  C:\\Users\\pcash\\anaconda3\\envs\\backtest\\python.exe tools\\build\\build_derived_bars_1m.py

  # Force-rebuild all dates (re-processes everything):
  C:\\Users\\pcash\\anaconda3\\envs\\backtest\\python.exe tools\\build\\build_derived_bars_1m.py --force-rebuild

  # Only process one specific dataset:
  C:\\Users\\pcash\\anaconda3\\envs\\backtest\\python.exe tools\\build\\build_derived_bars_1m.py --dataset-id ES_BARS_1M
...
- "No derived_bars rows found": run tools/admin/add_bars_1m_config.py first.
...
def main() -> int:
    ...
    if not derived_rows:
        print(
            ...
            "Run tools/admin/add_bars_1m_config.py first.",
            file=sys.stderr,
        )
        return 1
